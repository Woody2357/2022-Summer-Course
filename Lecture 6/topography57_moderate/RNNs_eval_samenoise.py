...
def test(model, var):
    ...
    for start in range(trajectory_true.shape[0]//2-10, trajectory_true.shape[0]):
        ...
        trajectory_pred[start, 0, 0] = trajectory_pred[start-1, 0, 0] +  \
                                       ...
                                       noise[0, start - 1]
        # print(trajectory_true[start, 0, 0] - (trajectory_true[start-1, 0, 0] + dt * (trajectory_true[start-1, 0, 1] - damp * (trajectory_true[start-1, 0, 0]-Ueq))))
        # print('xiaoniao', trajectory_pred[start, 0, 0] - (trajectory_pred[start-1, 0, 0] + dt * (trajectory_pred[start-1, 0, 1] - damp * (trajectory_pred[start-1, 0, 0]-Ueq))))
        # print(trajectory_pred[start], trajectory_true[start])
        print(trajectory_pred[start, 0, 0], trajectory_true[start, 0, 0])
