...
def main():
    ...
    for epoch in range(500):
        train_loss = avg_loss(trainloader, model, criterion, optimizer, use_cuda=True)
        print(train_loss)
        avg_losses.update(train_loss, 1)
    logger.append([state['lr'], avg_losses.avg])
    logger.close()
...
def train(trainloader, model, criterion, optimizer, use_cuda):
    ...
    return loss_item #losses.avg

def avg_loss(trainloader, model, criterion, optimizer, use_cuda):
