...
class DataEncapsulator():
    ...
    def build_grid_data(self, data_x, data_y, index, cuda=True):
        if self.available_subs[index] <= 18:
            ...
            batch_size = data_x.size()[0]
            batch_x = data_x[:,::self.available_subs[self.ts],::self.available_subs[self.ts]][:,:self.grid_sizes[self.ts],:self.grid_sizes[self.ts],:].permute(0,3,1,2)
            batch_y = data_y[:,::self.available_subs[self.ts],::self.available_subs[self.ts]][:,:self.grid_sizes[self.ts],:self.grid_sizes[self.ts]].reshape(batch_size,self.grid_sizes[self.ts],self.grid_sizes[self.ts],1).permute(0,3,1,2)
            batch_x = torch.nn.functional.interpolate(batch_x, size=(self.grid_sizes[index],self.grid_sizes[index]), mode='bicubic', align_corners=True).reshape(batch_size, 2, self.grid_sizes[index], self.grid_sizes[index]).permute(0,2,3,1)
            batch_y = torch.nn.functional.interpolate(batch_y, size=(self.grid_sizes[index],self.grid_sizes[index]), mode='bicubic', align_corners=True).reshape(batch_size, 1, self.grid_sizes[index], self.grid_sizes[index]).permute(0,2,3,1)
        ...
        batch_x = torch.cat([batch_x.reshape(batch_size,self.grid_sizes[index],self.grid_sizes[index],2), grid.repeat(batch_size,1,1,1)], dim=3)
    ...
    def read_data(self, filename=None, build_grid=False):
        ...
        x_train1 = self.read_field('u')[:self.ntrain,:,:]
        x_train2 = self.read_field('ui')[:self.ntrain,:,:]
        y_train1 = self.read_field('a')[:self.ntrain,:,:]

        x_test1 = self.read_field('u')[-self.ntest:,:,:]
        x_test2 = self.read_field('ui')[-self.ntest:,:,:]
        y_test1 = self.read_field('a')[-self.ntest:,:,:]
        ...
        t_min, t_max = torch.min(x_train2), torch.max(x_train2)
        x_train2 = (x_train2 - t_min)/(t_max - t_min)
        # print("x_train2: ", torch.min(x_train2), torch.max(x_train2))

        t_min, t_max = torch.min(x_test2), torch.max(x_test2)
        x_test2 = (x_test2 - t_min)/(t_max - t_min)
        # print("x_test2: ", torch.min(x_test2), torch.max(x_test2))

        # merge
        x_train = torch.cat([x_train1.reshape(self.ntrain, grid_size, grid_size, 1), x_train2.reshape(self.ntrain, grid_size, grid_size, 1)], -1)
        x_test = torch.cat([x_test1.reshape(self.ntest, grid_size, grid_size, 1), x_test2.reshape(self.ntest, grid_size, grid_size, 1)], -1)
        y_train = y_train1
        y_test = y_test1
        ...
        if build_grid: # in this case, append the locations information
            ...
            x_train = torch.cat([x_train.reshape(self.ntrain,grid_size,grid_size,2), grid.repeat(self.ntrain,1,1,1)], dim=3)
            x_test = torch.cat([x_test.reshape(self.ntest,grid_size,grid_size,2), grid.repeat(self.ntest,1,1,1)], dim=3)
    ...
    def build_data(self, sub=1, target_sub=1, ntrain=10000, ntest=1000, load_type="default"):
        ...
        self.input_channel = 4 # (a(x, y), x, y)
